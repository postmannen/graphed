...
func BenchmarkAddNodes(b *testing.B) {
	...
	store, err := NewNodeStore(dataDir, WithChunkSize(chunkSize))
	if err != nil {
...
func benchmarkGetChild(nodeName string, store *NodeStore) (string, bool) {
	nd, err := store.GetNodeByName(nodeName)
	if err != nil {
		log.Printf("Failed to retrieve node: %v : %v\n", nodeName, err)
	}
...
func addBenchmarkNodes(b *testing.B, store *NodeStore) {
...
func BenchmarkComplexNodeRetrieval(b *testing.B) {
	...
	store, err := NewNodeStore(dataDir, WithChunkSize(100))
	if err != nil {
		b.Fatalf("Failed to create store: %v", err)
	}
	defer store.Close()
